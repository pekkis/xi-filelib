...
class SymfonyRendererTest extends \Xi\Filelib\Tests\TestCase
{
    ...
    {
        return new SymfonyRenderer($this->configuration, $this->fiop);

        $renderer = $this
            ->getMockBuilder('Xi\Filelib\Renderer\SymfonyRenderer')
            ->setMethods($methods)
            ->setConstructorArgs(array($this->configuration, $this->fiop))
            ->getMock();
    ...
    {
         $renderer = new SymfonyRenderer($this->configuration, $this->fiop);
    ...
    public function acceleratedRequestShouldBeEmptyAndContainCorrectHeaders($expectedHeader, $serverSignature)
    {
        ...
        $this->configuration->setStorage($this->storage);
